...
Status: Accepted direction, implementation validation pending
...
As of the March 2026 Igalia WPEPlatform launcher write-up, WPEPlatform stable release timing was planned for September 2026 and the legacy API was expected to be deprecated after that point. Treat this as a dated upstream signal, not as a verified Buildroot availability guarantee.
...
- Confirm the first Buildroot release or upstream Buildroot commit that can build WPE WebKit with WPEPlatform.
- Re-check the WPEPlatform stable release and legacy-deprecation status before pinning a release branch.
...
- Confirm whether the dashboard launcher should be C, C++, or another GObject-friendly language once package size, cross-build friction, and startup time are measured.
